...
print(image2.size)
print(data)
print("Виберіть варіант \n1.Зміна кольору зображення\n2.Зміна розміру")
number = int(input("Ваш вибір:"))
if (number == 1):
    im = np.array(Image.open('Photo.jpg').convert('L'))
    print(type(im))
    gr_im = Image.fromarray(im).save('grPhoto.png')
if (number == 2):
    load_img_rz = np.array(Image.open('Photo.jpg').resize((200, 200)))
...
def Graf():
    ...
    data_valuesg = [np.count_nonzero(green25), np.count_nonzero(green50), np.count_nonzero(green75),
                    np.count_nonzero(green100),
                    np.count_nonzero(green125), np.count_nonzero(green150), np.count_nonzero(green175),
                    np.count_nonzero(green200),
                    np.count_nonzero(green225), np.count_nonzero(green255)]
    data_valuesb = [np.count_nonzero(blue25), np.count_nonzero(blue50), np.count_nonzero(blue75),
                    np.count_nonzero(blue100),
                    np.count_nonzero(blue125), np.count_nonzero(blue150), np.count_nonzero(blue175),
                    np.count_nonzero(blue200),
